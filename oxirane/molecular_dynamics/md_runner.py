...
isolated_model_path = "oxirane/molecular_dynamics/isolated_model/oxirane_isolated_compiled.model"
device = 'cuda' if torch.cuda.is_available() else 'cpu'

sampled_angles = [50.0, 60.0, 70.0, 80.0, 90.0, 100.0, 110.0, 120.0, 130.0, 140.0]
snapshots = 500
...
for angle in sampled_angles:        
    atoms_path = f"oxirane/isolated_system/geometry_optimisations/angle_{angle}/orca.xyz"
    atoms = read(atoms_path)
    atoms.calc = calculator
    dyn = Langevin(atoms, timestep = 0.5 *fs, temperature_K = 300, friction = 0.01/fs)
    logfile = f"oxirane/molecular_dynamics/logs/angle_{angle}"
    snapshot_dir = f"oxirane/molecular_dynamics/snapshots/angle_{angle}"
...
xyz_dir = 'oxirane/molecular_dynamics/snapshots'
...
for state in range(1, num_states + 1):
    for snapshot in snapshots_dir:
        calc_dir = f'oxirane/molecular_dynamics/snapshots_energies/sf-es_{state}/{snapshot}'
